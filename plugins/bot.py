...
async def lol(ult):
    pic = mrunal.get("ALIVE_PIC")
    uptime = time_formatter((time.time() - start_time) * 1000)
    header = mrunal.get("ALIVE_TEXT") or get_string("bot_1")
...
@beastx_cmd(pattern="ping$", chats=[], type=["official", "assistant"])
async def _(event):
    if event.out:
        await event.delete()
    start = time.time()
...
async def restartbt(ult):
    ...
    os.execl(sys.executable, sys.executable, "-m", "py-beastx")
