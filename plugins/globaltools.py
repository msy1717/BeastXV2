...
from py-beastx.dB import DEVLIST
from py-beastx.dB.gban_mute_db import (
    gban,
    gmute,
    is_gbanned,
    is_gmuted,
...
from py-beastx.dB.gcast_blacklist_db import (
    add_gblacklist,
    is_gblacklisted,
    rem_gblacklist,
)
...
@beastx_cmd(pattern="gpromote ?(.*)", fullsudo=True)
async def _(e):
...
@beastx_cmd(pattern="gdemote ?(.*)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1)
    beastx_bot = e.client
    if not x:
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await beastx_bot.get_entity(user)
        except BaseException:
            return await eor(e, f"`No User Found Regarding {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in beastx_bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@beastx_cmd(pattern="ungban ?(.*)", fullsudo=True)
async def _(e):
...
@beastx_cmd(pattern="gban ?(.*)", fullsudo=True)
async def _(e):
...
@beastx_cmd(pattern="g(admin|)cast ?(.*)", fullsudo=True)
...
@beastx_cmd(pattern="gucast ?(.*)", fullsudo=True)
...
@beastx_cmd(pattern="gkick ?(.*)", fullsudo=True)
async def gkick(e):
...
@beastx_cmd(pattern="gmute ?(.*)", fullsudo=True)
async def _(e):
...
@beastx_cmd(pattern="ungmute ?(.*)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    for i in users:
        try:
            name = (await beastx_bot.get_entity(int(i))).first_name
        except BaseException:
            name = i
...
@beastx_cmd(pattern="gblacklist")
async def blacklist_(event):
    await gblacker(event, "add")


@beastx_cmd(pattern="ungblacklist")
...
async def gblacker(event, type_):
    ...
    try:
        ...
        pass
    try:
        chat_id = (await beastx_bot.get_entity(chat)).id
