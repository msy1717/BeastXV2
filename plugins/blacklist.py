...
from py-beastx.dB.blacklist_db import (
    add_blacklist,
    get_blacklist,
    list_blacklist,
    rem_blacklist,
...
from . import eor, events, get_string, beastx_bot, beastx_cmd


@beastx_cmd(pattern="blacklist ?(.*)", admins_only=True)
async def af(e):
...
@beastx_cmd(pattern="remblacklist ?(.*)", admins_only=True)
async def rf(e):
    wrd = e.pattern_match.group(1)
    chat = e.chat_id
    if not wrd:
...
@beastx_cmd(pattern="listblacklist$", admins_only=True)
async def lsnote(e):
