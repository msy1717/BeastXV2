...
from py-beastx.functions.admins import ban_time
...
@beastx_cmd(pattern="promote ?(.*)", admins_only=True, type=["official", "manager"])
async def prmte(ult):
...
@beastx_cmd(pattern="tban ?(.*)", type=["official", "manager"])
async def tkicki(e):
    huh = e.text.split(" ")
    try:
        tme = huh[1]
...
@beastx_cmd(pattern="pin$", type=["official", "manager"])
async def pin(msg):
    if not msg.is_reply:
...
async def fastpurgerme(purg):
    ...
    async for msg in purg.client.iter_messages(
        ...
        if len(msgs) == 100:
            await beastx_bot.delete_messages(chat, msgs)
            msgs = []
...
@beastx_cmd(pattern="pinned", type=["official", "manager"], groups_only=True)
