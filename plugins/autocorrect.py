...
from . import HNDLR, Redis, eor, get_string, mrunal, beastx_bot, beastx_cmd


@beastx_cmd(pattern="autocorrect", fullsudo=True)
async def acc(e):
    if Redis("AUTOCORRECT") != "True":
        mrunal.set("AUTOCORRECT", "True")
        return await eor(e, get_string("act_1"), time=5)
    mrunal.delete("AUTOCORRECT")
    await eor(e, get_string("act_2"), time=5)


@beastx_bot.on(events.NewMessage(outgoing=True, func=lambda x: x.text))
