...
from py-beastx.dB.afk_db import add_afk, del_afk, is_afk
from py-beastx.dB.pmpermit_db import is_approved
...
@beastx_cmd(pattern="afk ?(.*)", fullsudo=True)
async def set_afk(event):
    ...
    if text and media:
        if "sticker" in media_type:
            msg1 = await beastx_bot.send_file(event.chat_id, file=media)
            msg2 = await beastx_bot.send_message(
                event.chat_id, get_string("afk_5").format(text)
            )
        else:
            msg1 = await beastx_bot.send_message(
                event.chat_id, get_string("afk_5").format(text), file=media
            )
    elif media:
        if "sticker" in media_type:
            msg1 = await beastx_bot.send_file(event.chat_id, file=media)
            msg2 = await beastx_bot.send_message(event.chat_id, get_string("afk_6"))
        else:
            msg1 = await beastx_bot.send_message(
...
@beastx_bot.on(events.NewMessage(outgoing=True))
