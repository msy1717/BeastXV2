...
from py-beastx.dB.clean_db import add_clean, rem_clean

from . import eor, get_string, mrunal, beastx_cmd


@beastx_cmd(pattern="addclean$", admins_only=True)
async def _(e):
    add_clean(e.chat_id)
    await eor(e, get_string("clan_1"), time=5)


@beastx_cmd(pattern="remclean$")
async def _(e):
    rem_clean(e.chat_id)
    await eor(e, get_string("clan_2"), time=5)


@beastx_cmd(pattern="listclean$")
async def _(e):
