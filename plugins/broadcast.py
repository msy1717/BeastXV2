...
from py-beastx.dB.broadcast_db import *

from . import HNDLR, LOGS, eor, get_string, mrunal, beastx_bot, beastx_cmd
...
async def broadcast_adder(event):
    ...
    if int(chat_id) == int(mrunal.get("LOG_CHANNEL")):
        return
...
async def forw(event):
    ...
    for channel in channels:
        try:
            await beastx_bot.forward_messages(int(channel), previous_message)
            sent_count += 1
            await x.edit(
                f"Sent : {sent_count}\nError : {error_count}\nTotal : {len(channels)}",
            )
    ...
    if error_count > 0:
        await beastx_bot.send_message(
            int(mrunal.get("LOG_CHANNEL")), f"{error_count} Errors"
        )
...
async def sending(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message:
            ...
            for channel in channels:
                try:
                    await beastx_bot.send_message(int(channel), previous_message)
                    sent_count += 1
                    await x.edit(
                        f"Sent : {sent_count}\nError : {error_count}\nTotal : {len(channels)}",
                    )
