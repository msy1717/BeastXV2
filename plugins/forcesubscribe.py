...
from py-beastx.dB.forcesub_db import add_forcesub, get_forcesetting, rem_forcesub
...
@beastx_cmd(pattern="fsub ?(.*)", admins_only=True, groups_only=True)
async def addfor(e):
...
async def fcall(e):
    match = e.pattern_match.group(1)
    spli = match.split("_")
    user = await beastx_bot.get_entity(int(spli[0]))
    cl = await beastx_bot.get_entity(int(spli[1]))
    text = f"Hi [{user.first_name}](tg://user?id={user.id}), You Need to Join"
    text += f" {cl.title} in order to Chat in this Group."
    if not cl.username:
        el = (await beastx_bot(ExportChatInviteRequest(cl))).link
    else:
...
async def cacheahs(ult):
    ...
    if count in range(2, 11):
        return
    try:
        await beastx_bot.get_permissions(int(joinchat), user.id)
        return
    except UserNotParticipantError:
        pass
    try:
        await beastx_bot.edit_permissions(ult.chat_id, user.id, send_messages=False)
    except ChatAdminRequiredError:
        return
    except Exception as e:
        LOGS.info(e)
    res = await beastx_bot.inline_query(asst.me.username, f"fsub {user.id}_{joinchat}")
