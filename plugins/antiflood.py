...
from py-beastx.dB.antiflood_db import get_flood, get_flood_limit, rem_flood, set_flood
...
async def unmuting(e):
    ...
    user = int(ino[0])
    chat = int(ino[1])
    user_name = (await beastx_bot.get_entity(user)).first_name
    chat_title = (await beastx_bot.get_entity(chat)).title
    await beastx_bot.edit_permissions(chat, user, send_messages=True)
    await e.edit(
