...
from . import eor, get_string, get_user_id, mrunal, beastx_bot, beastx_cmd
...
async def _(event):
    ...
    if mrunal.get("FBAN_GROUP_ID"):
        chat = int(mrunal.get("FBAN_GROUP_ID"))
    else:
        ...
        for a in range(3):
            async with event.client.conversation("@MissRose_bot") as bot_conv:
                ...
                if "make a file" in response.text or "Looks like" in response.text:
                    ...
                    await asyncio.sleep(3)
                    if fedfile.media:
                        downloaded_file_name = await beastx_bot.download_media(
                            fedfile,
                            "fedlist",
                        )
    ...
    try:
        await beastx_bot.send_message(chat, "/start")
    except BaseException:
        return await msg.edit(get_string("sf_11"))
    await asyncio.sleep(3)
    if mrunal.get("EXCLUDE_FED"):
        excludeFed = mrunal.get("EXCLUDE_FED").split(" ")
        for num, item in enumerate(excludeFed):
            excludeFed[num] = item.strip()
    exCount = 0
    for fed in fedList:
        if mrunal.get("EXCLUDE_FED") and fed in excludeFed:
            await beastx_bot.send_message(chat, f"{fed} Excluded.")
            exCount += 1
            continue
...
async def _(event):
    ...
    for fed in fedList:
        if mrunal.get("EXCLUDE_FED") and fed in excludeFed:
            ...
            exCount += 1
            continue
        await beastx_bot.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await beastx_bot.send_message(chat, f"/unfban {FBAN} {REASON}")
        await asyncio.sleep(3)
    try:
        os.remove("fedlist")
    except Exception as e:
    ...
    if sysarg == "":
        ...
        async with event.client.conversation(bot) as conv:
            try:
                ...
                await beastx_bot.send_read_acknowledge(bot)
