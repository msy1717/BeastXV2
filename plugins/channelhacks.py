...
from py-beastx.dB.ch_db import (
    add_destination,
    add_source_channel,
    get_destinations,
    get_no_destinations,
...
from . import asst, eor, events, get_string, mrunal, beastx_bot, beastx_cmd


@beastx_bot.on(events.NewMessage())
async def _(e):
    if mrunal.get("AUTOPOST") != "True":
        return
    ...
    for ys in y:
        try:
            await beastx_bot.send_message(int(ys), e.message)
        except Exception as ex:
            await asst.send_message(int(mrunal["LOG_CHANNEL"]), str(ex))


@beastx_cmd(pattern="shift (.*)")
async def _(e):
...
async def source(e):
    ...
    try:
        ...
        try:
            y = int((await beastx_bot.get_entity(x)).id)
...
@beastx_cmd(pattern="dsource ?(.*)")
async def dd(event):
    chat_id = event.pattern_match.group(1)
    x = await eor(event, get_string("com_1"))
    if chat_id == "all":
        await x.edit(get_string("bd_8"))
        mrunal.delete("CH_SOURCE")
...
@beastx_cmd(pattern="ddest ?(.*)")
async def dd(event):
    chat_id = event.pattern_match.group(1)
    x = await eor(event, get_string("com_1"))
    if chat_id == "all":
        await x.edit(get_string("bd_8"))
        mrunal.delete("CH_DESTINATION")
...
@beastx_cmd(pattern="listdest")
async def list_all(event):
    beastx_bot = event.client
    x = await eor(event, get_string("com_1"))
    ...
    for channel in channels:
        name = ""
        try:
            name = (await beastx_bot.get_entity(int(channel))).title
        except BaseException:
            name = ""
        msg += f"\n=> **{name}** [`{channel}`]"
