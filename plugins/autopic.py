...
from py-beastx.functions.misc import unsplashsearch
from telethon.tl.functions.photos import UploadProfilePhotoRequest

from . import download_file, eor, get_string, mrunal, beastx_cmd


@beastx_cmd(pattern="autopic ?(.*)")
async def autopic(e):
    ...
    mrunal.set("AUTOPIC", "True")
    ST = mrunal.get("SLEEP_TIME")
    SLEEP_TIME = int(ST) if ST else 1221
    while True:
        for lie in clls:
            if mrunal.get("AUTOPIC") != "True":
                return
...
@beastx_cmd(pattern="stoppic$")
async def stoppo(ult):
    gt = mrunal.get("AUTOPIC")
    if gt != "True":
        return await eor(ult, get_string("autopic_4"), time=5)
    mrunal.set("AUTOPIC", "None")
