...
from py-beastx.functions.gdrive import authorize, create_token_file
from py-beastx.functions.tools import get_paste, telegraph_client
from py-beastx.startup.loader import Loader
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "py-beastx")
...
async def _(e):
    if not e.is_private:
        return
    if not mrunal.get("GDRIVE_CLIENT_ID"):
        return await e.edit(
    ...
    mrunal.set("GDRIVE_TOKEN", token_file_data)
    await e.reply(
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        repl = await reply
        mrunal.set("GDRIVE_FOLDER_ID", repl.text)
        await repl.reply(
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        reply = conv.wait_event(events.NewMessage(from_users=e.sender_id))
        repl = await reply
        mrunal.set("GDRIVE_CLIENT_SECRET", repl.text)
        await repl.reply(
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        mrunal.set("GDRIVE_CLIENT_ID", repl.text)
        await repl.reply(
...
async def rhwhe(e):
    if mrunal.get("DUAL_MODE"):
        mrunal.delete("DUAL_MODE")
        key = "Off"
    else:
        mrunal.set("DUAL_MODE", "True")
        key = "On"
    Msg = "Dual Mode : " + key
...
async def hhh(e):
    async with e.client.conversation(e.chat_id) as conv:
        ...
        mrunal.set("STARTMEDIA", msg.file.id)
...
async def hhh(e):
    async with e.client.conversation(e.chat_id) as conv:
        ...
        mrunal.set("BOT_INFO_START", msg.text)
        await conv.send_message("Done!", buttons=get_back_button("chatbot"))
