...
@beastx_cmd(pattern="decide", type="manager")
async def dheh(e):
...
@beastx_cmd(pattern="echo ?(.*)", type=["manager"])
async def oqha(e):
...
@beastx_cmd(pattern="kickme$", type=["manager"], allow_all=True)
async def doit(e):
...
@beastx_cmd(pattern="joke$", type=["manager"])
async def do_joke(e):
