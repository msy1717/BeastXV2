...
from py-beastx.dB.asst_fns import *
from py-beastx.dB.sudos import is_fullsudo
from py-beastx.functions.helper import inline_mention
...
Owner_info_msg = (
    mrunal.get("BOT_INFO_START")
    or f"""
**Owner** - {OWNER_NAME}
**OwnerID** - `{OWNER_ID}`

**Message Forwards** - {mrunal.get("PMBOT")}
...
async def ultroid(event):
    if not is_added(event.sender_id) and str(event.sender_id) not in owner_and_sudos():
        ...
        kak_uiw = mrunal.get("OFF_START_LOG")
        if not kak_uiw or kak_uiw != "True":
            ...
            await event.client.send_message(
                int(mrunal["LOG_CHANNEL"]), msg, buttons=buttons
            )
    if (event.sender_id != OWNER_ID) and not is_fullsudo(event.sender_id):
        ok = ""
        u = await event.client.get_entity(event.chat_id)
        if not mrunal.get("STARTMSG"):
            if mrunal.get("PMBOT") == "True":
                ok = "You can contact my master using this bot!!\n\nSend your Message, I will Deliver it To Master."
            await event.reply(
                f"Hey there [{get_display_name(u)}](tg://user?id={u.id}), this is Ultroid Assistant of [{beastx_bot.me.first_name}](tg://user?id={beastx_bot.uid})!\n\n{ok}",
                file=mrunal.get("STARTMEDIA"),
                buttons=[Button.inline("Info.", data="ownerinfo")]
                if Owner_info_msg != "False"
                else None,
            )
        else:
            me = f"[{beastx_bot.me.first_name}](tg://user?id={beastx_bot.uid})"
            mention = f"[{get_display_name(u)}](tg://user?id={u.id})"
            await event.reply(
                mrunal.get("STARTMSG").format(me=me, mention=mention),
                file=mrunal.get("STARTMEDIA"),
                buttons=[Button.inline("Info.", data="ownerinfo")],
            )
    else:
